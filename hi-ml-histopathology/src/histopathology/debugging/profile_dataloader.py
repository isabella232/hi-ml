...
from histopathology.models.transforms import TimerTransform
...
def load_dataset(slides_dataset: SlidesDataset, strat: str = "slides") -> Dataset:
    ...
    if strat == "slides":
        ...
        training_list = load_decathlon_datalist(
            data_list_file_path="/home/t-kbouzid/workspace/repos/hi-ml/hi-ml-histopathology/src/histopathology/debugging/csv.json",
...
def main() -> None:
    ...
    for i, batch in enumerate(dataloader):
        ...
        print("TileOnGridd took {:.2f}s per slide".format(batch['TileOnGridd'][0].mean()))
        print("TileOnGridd took {:.2f}s for the entire batch".format(batch['TileOnGridd'][0].sum()))
        print(f"TileOnGridd took {batch['TileOnGridd'][0]}")

        print("LoadImaged took {:.2f}s per slide".format(batch['LoadImaged'][0].mean()))
        print("LoadImaged took {:.2f}s for the entire batch".format(batch['LoadImaged'][0].sum()))
        print(f"LoadImaged took {batch['LoadImaged'][0]}")
        start = time.time()
    #     prof.step()
    # prof.stop()
